...
struct Ext2FSInode * Ext2FS::load_inode(unsigned int inode_number)
{
	...
	int grupo = blockgroup_for_inode(inode_number);
	struct  Ext2FSBlockGroupDescriptor * blockGroup = block_group(grupo);

	int idx_grupo = blockgroup_inode_index(inode_number);

	//calculo direccion del inodo
	unsigned int table_offset = idx_grupo * sizeof(Ext2FSInode);
	unsigned int inode_table_block = blockGroup->inode_table + table_offset / block_size;
	unsigned int offset_in_block = table_offset % block_size;

	//obtengo bloque del nodo
	unsigned char * bloque = (unsigned char*) malloc(block_size);
	read_block(inode_table_block, bloque);

	return ((struct Ext2FSInode *)(bloque + offset_in_block));
}

unsigned int Ext2FS::get_block_address(struct Ext2FSInode * inode, unsigned int block_number)
{
	int block_size = 1024 << _superblock->log_block_size;
	int table_size = block_size / 4;
	unsigned int block_addr;

	if(block_number < inode->blocks){
		printf("No existe bloque al que acceder");
		return 1;
	}

	if(block_number < 12){
		block_addr = inode->block[block_addr];

		return block_addr;
	}

	if(12 <= block_number < table_size + 12){
		int index = block_number - 12;
		unsigned char * table = (unsigned char *)malloc(block_size);
		
		// Busco la tabla de direcciones de bloques
		read_block(inode->block[12], table);

		block_addr = table[index];
	...
	if(table_size + 12 <= block_number < (table_size^2 + table_size + 12)){
		int index_first_table = (block_number - 12 - table_size) / table_size;
		int index_second_table = (block_number - 12 - table_size) % table_size;
		unsigned char * first_table = (unsigned char *)malloc(block_size);
		unsigned char * second_table = (unsigned char *)malloc(block_size);

		// Leo la tabla indirecta doble
		read_block(inode->block[13], first_table);

		read_block(first_table[index_second_table], second_table);

		block_addr = second_table[index_second_table];
